...
<head>
    ...
    <title>Lembar Pemantauan Fibrinolitik</title>
    ...
    <div class="container">
        <div class="modal fade" id="modal_simpan" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <form method="post" onsubmit="return cek_form(this)" action="{{ url('e_rekam_medis/detail/save_lembar_pemantauan_fibrinolitik') }}">
                        @csrf
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <input type="text" name="tgl_tindakan" id="tgl_tindakan" autocomplete="off" class="fulldate form-control">
                            </div>
                            <div class="form-group">
                                <label for="">Skala Nyeri</label>
                                <input type="text" name="skala_nyeri" id="skala_nyeri" class="form-control">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <label for="">Pendarahan Aktif -/+</label>
                                <input type="text" name="pendarahan" id="pendarahan" class="form-control">
                            </div>
                        </div>
        ...
        <div class="modal fade" id="modal_petugas" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <form method="post" action="{{ url('e_rekam_medis/detail/verifikasi_lembar_pemantauan_fibrinolitik') }}">
                        @csrf
                        <input type="hidden" name="dokumen" value="{{$dokumen->id}}">
        ...
        <div class="container mt-3">
            <table class="table table-bordered table-0 custom-table">
                <thead>
                  <tr>
                    <th scope="col" colspan="10" class="text-center" style="background-color: lightgrey; margin-bottom:0; border-color: black;">
                ...
                <tbody>
                    <tr class="text-center mt-2" >
                        <td class="font-weight-bold" style="vertical-align: middle; width: 10%">Tanggal / Jam</td>
                        <td class="font-weight-bold" style="vertical-align: middle">Skala Nyeri Dada 1-10</td>
                        <td class="font-weight-bold" style="vertical-align: middle">Tensi</td>
                        <td class="font-weight-bold" style="vertical-align: middle">Nadi</td>
                        <td class="font-weight-bold" style="vertical-align: middle">RR</td>
                        <td class="font-weight-bold" style="vertical-align: middle">SPO2</td>
                        <td class="font-weight-bold" style="vertical-align: middle">Pendarahan Aktif -/+</td>
                        <td class="font-weight-bold" style="vertical-align: middle">Nama</td>
                        <td class="font-weight-bold" style="vertical-align: middle">Paraf</td>
                    ...
                    @if (isset($data))
                        @foreach ($data as $item)
                            <tr>
                                <td style="vertical-align: middle; text-align: center" id="isian_tgl_tindakan_{{ $item->id }}">{{ date('d-m-Y H:i', strtotime($item->tgl_tindakan)) }}</td>
                                <td style="vertical-align: middle; text-align: center" id="isian_skala_nyeri_{{ $item->id }}">{{ $item->skala_nyeri }}</td>
                                <td style="vertical-align: middle; text-align: center" id="isian_tensi_{{ $item->id }}">{{ $item->tensi }}</td>
                                <td style="vertical-align: middle; text-align: center" id="isian_nadi_{{ $item->id }}">{{ $item->nadi }}</td>
                                <td style="vertical-align: middle; text-align: center" id="isian_rr_{{ $item->id }}">{{ $item->rr }}</td>
                                <td style="vertical-align: middle; text-align: center" id="isian_spo2_{{ $item->id }}">{{ $item->spo2 }}</td>
                                <td style="vertical-align: middle; text-align: center" id="isian_pendarahan_{{ $item->id }}">{{ $item->pendarahan }}</td>
                                <td style="text-align: center; vertical-align: middle" onclick="open_modal_petugas(`{{ $item->id }}`)">
                                ...
                                <td style="text-align: center; vertical-align: middle">
                                    ...
                                    <a onclick="if (confirm('Apakah Anda yakin akan menghapus?')){return true;}else{event.stopPropagation(); event.preventDefault();};" href="{{ url('e_rekam_medis/detail/hapus_lembar_pemantauan_fibrinolitik') }}/{{ $item->id }}" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></a>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
...
<script>
    ...
    function open_modal_simpan(id) {
        if (id != 0) {
            ...
            $('#skala_nyeri').val($('#isian_skala_nyeri_'+id).html());
            $('#tensi').val($('#isian_tensi_'+id).html());
            $('#nadi').val($('#isian_nadi_'+id).html());
            $('#rr').val($('#isian_rr_'+id).html());
            $('#spo2').val($('#isian_spo2_'+id).html());
            $('#pendarahan').val($('#isian_pendarahan_'+id).html());
        }
