<form id="form-{{ Illuminate\Support\Str::slug($renpra->id.' '.$renpra->nama_renpra, '-') }}" class="form-renpra w-100" method="post" autocomplete="off">
    ...
    <table class="w-100 table-renpra mt-1">
        ...
        <tr class="align-top">
            ...
            <td class="p-1">
                <p><strong>Perfusi jaringan kardiopulmonal tidak efektif</strong> b/d gangguan afinitas Hb oksigen, penurunan konsentrasi Hb, Hipervolemia, Hipoventilasi, gangguan transport O2, gangguan aliran arteri dan vena.</p>
                <p>DS:</p>
            ...
            <td class="p-1">
                <p>Setelah dilakukan asuhan selama <input type="text" name="lama_tindakan" id="{{ $renpra->id }}-lama_tindakan" class="form-control rounded-0 input-dotted" value="{{ $renpra->lama_tindakan }}" required /> ketidakefektifan perfusi jaringan kardiopulmonal teratasi dengan kriteria hasil:</p>
                <ul class="list-unstyled">
                    @foreach([
    ...
    <div class="w-100 text-right mb-2">RSHM/RI/58.01/Rev.00</div>
</form>