<form id="form-{{ Illuminate\Support\Str::slug($renpra->nama_renpra, '-') }}" class="form-renpra w-100" method="post" autocomplete="off">
    ...
    <table class="w-100 table-renpra mt-1">
        ...
        <tr class="align-top">
            ...
            <td class="p-1">
                ...
                <p>Biofisika (penyakit kronis), kognitif/persepsi (nyeri kronis), kultural/spiritual, penyakit, krisis situasional, trauma/injury, pengobatan (pembedahan, kemoterapi, radiasi).</p>
                <p></p>
                <p>DS :</p>
            ...
            <td class="p-1">
                <p>Setelah dilakukan tindakan keperawatan selama <input type="text" name="lama_tindakan" id="{{ $renpra->id }}-lama_tindakan" class="form-control rounded-0 input-dotted" value="{{ $renpra->lama_tindakan }}" required /> gangguan body image pasien teratasi dengan kriteria hasil :</p>
                <ul class="list-unstyled">
                    @foreach(
                    [
                    'Body image positif',
    ...
    <div class="w-100 text-right mb-2">RSHM/RI/39.01/Rev.00</div>
</form>
