...
<div class="modal fade" id="modal_ttd" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            $hubungan_lain=False;
            $hubungan='';
            if (isset($dokumen->tindakan_anestesi_epidural)) {
                if($dokumen->tindakan_anestesi_epidural->hubungan != "Saya Sendiri" && $dokumen->tindakan_anestesi_epidural->hubungan != "Suami" &&
                    $dokumen->tindakan_anestesi_epidural->hubungan != "Istri" && $dokumen->tindakan_anestesi_epidural->hubungan != "Anak" &&
                    $dokumen->tindakan_anestesi_epidural->hubungan != "Ayah" && $dokumen->tindakan_anestesi_epidural->hubungan != "Ibu"){
                    $hubungan_lain=True;
                    $hubungan=$dokumen->tindakan_anestesi_epidural->hubungan;
                }
            }
            @endphp
            <form method="POST" onsubmit="return konfirmasi_ttd(this)"
                action="{{ url('e_rekam_medis/detail/ttd_tindakan_anestesi_spinal_atau_epidural') }}">
...
<body class="p-2">
    ...
    <table class="w-100">
        ...
        <div class="container mt-3">
            ...
            <form method="post" onsubmit="return cek_pemberi_informasi()" action="{{url('e_rekam_medis/detail/save_tindakan_anestesi_spinal_atau_epidural')}}">
            @csrf
            ...
            <input type="hidden" name="checklist" id="checklist">
                ...
                <tbody>
                    ...
                    <td colspan="2">
                        <select name="id_dokter_pelaksana" id="id_dokter_pelaksana" class="form-control select2" style="width: 100%">
                            ...
                            @foreach ($dokter_pelaksana as $dokter)
                                <option value="{{$dokter->id}}" {{$dokumen->tindakan_anestesi_epidural&&$dokumen->tindakan_anestesi_epidural->id_dokter_pelaksana==$dokter->id?'selected':''}}>{{ $dokter->nama }}</option>
                            @endforeach
                        </select>
                    </td>
                  </tr>
                    ...
                    <td colspan="2">
                        <select name="id_pemberi_informasi" id="id_pemberi_informasi" class="form-control select2" style="width: 100%">
                            ...
                            @foreach ($pemberi_informasi as $pemberi_info)
                                <option value="{{ $pemberi_info->id }}" {{$dokumen->tindakan_anestesi_epidural&&$dokumen->tindakan_anestesi_epidural->id_pemberi_informasi==$pemberi_info->id?'selected':''}}>{{ $pemberi_info->nama }}</option>
                            @endforeach
                        </select>
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td colspan="2">
                        <input type="text" name="penerima_informasi" class="form-control" value="{{$dokumen->tindakan_anestesi_epidural?$dokumen->tindakan_anestesi_epidural->penerima_informasi:$dokumen->nama_pasien}}" required>
                    </td>
                  </tr>
                  ...
                  <tr>
                    ...
                    <td >Diagnosa</td>
                    <td><select name="id_diagnosa" id="id_diagnosa" class="form-control select2" style="width: 100%">
                        <option value="{{$dokumen->tindakan_anestesi_epidural?$dokumen->tindakan_anestesi_epidural->indikasi_tindakan:''}}" selected disabled>Select item...</option>
                            @foreach ($diagnosa as $icd)
                                <option value="{{ $icd->id }}" {{$dokumen->tindakan_anestesi_epidural && $dokumen->tindakan_anestesi_epidural->id_diagnosa==$icd->id?'selected':''}}>{{$icd->icd .'-'. $icd->nama }}</option>
                            @endforeach
                        </select>
                    </td>
                    <td>
                        <input type="checkbox" id="satu" {{ $dokumen->tindakan_anestesi_epidural ? (in_array('satu', json_decode($dokumen->tindakan_anestesi_epidural->checklist)) ?'checked' : '') : '' }} class="form-control">
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td >Dasar Diagnosis</td>
                        <td>
                            <input type="checkbox" id="dasar_satu" {{ $dokumen->tindakan_anestesi_epidural && isset($dokumen->tindakan_anestesi_epidural->dasar_diagnosis)  
                            ? (in_array('satu',json_decode($dokumen->tindakan_anestesi_epidural->dasar_diagnosis))?'checked' : '') : '' }}> Anamnesis,
                            <input type="checkbox" id="dasar_dua" {{ $dokumen->tindakan_anestesi_epidural && isset($dokumen->tindakan_anestesi_epidural->dasar_diagnosis)
                            ? (in_array('dua',json_decode($dokumen->tindakan_anestesi_epidural->dasar_diagnosis))?'checked' : '') : '' }}> Pemeriksaan Fisik,
                            <input type="checkbox" id="dasar_tiga" {{ $dokumen->tindakan_anestesi_epidural && isset($dokumen->tindakan_anestesi_epidural->dasar_diagnosis)
                            ? (in_array('tiga',json_decode($dokumen->tindakan_anestesi_epidural->dasar_diagnosis))?'checked' : '') : '' }}> Hasil Pemeriksaan Laboratorium,
                            <input type="checkbox" id="dasar_empat" {{ $dokumen->tindakan_anestesi_epidural && isset($dokumen->tindakan_anestesi_epidural->dasar_diagnosis)
                            ? (in_array('empat',json_decode($dokumen->tindakan_anestesi_epidural->dasar_diagnosis))?'checked' : '') : '' }}> EKG,<br>
                            <input type="checkbox" id="dasar_lima" {{ $dokumen->tindakan_anestesi_epidural && isset($dokumen->tindakan_anestesi_epidural->dasar_diagnosis)
                            ? (in_array('lima',json_decode($dokumen->tindakan_anestesi_epidural->dasar_diagnosis))?'checked' : '') : '' }}> Pemeriksaan checkboxlogi (Toraks, MRI, USG, dll),
                            <input type="checkbox" id="dasar_enam" {{ $dokumen->tindakan_anestesi_epidural && isset($dokumen->tindakan_anestesi_epidural->dasar_diagnosis)
                            ? (in_array('empat',json_decode($dokumen->tindakan_anestesi_epidural->dasar_diagnosis))?'checked' : '') : '' }}> Lain-lain
                        </td>
                        <td>
                            <input type="checkbox" id="dua" {{ $dokumen->tindakan_anestesi_epidural ? (in_array('dua',json_decode($dokumen->tindakan_anestesi_epidural->checklist))?'checked' : '') : '' }} class="form-control">
                        </td>
                  </tr>
                  <tr>
                    ...
                    <td>
                        <input type="checkbox" id="tiga" {{ $dokumen->tindakan_anestesi_epidural ? (in_array('tiga',json_decode($dokumen->tindakan_anestesi_epidural->checklist))?'checked' : '') : '' }} class="form-control">
                    </td>
                  </tr>
                  <tr>
                    <td class="text-center">4</td>
                    <td>Indikasi Tindakan</td>
                    <td><input type="text" name="indikasi_tindakan" class="form-control" value="{{$dokumen->tindakan_anestesi_epidural?$dokumen->tindakan_anestesi_epidural->indikasi_tindakan:''}}"></td>
                    <td>
                        <input type="checkbox" id="empat" {{ $dokumen->tindakan_anestesi_epidural ? (in_array('empat',json_decode($dokumen->tindakan_anestesi_epidural->checklist))?'checked' : '') : '' }} class="form-control">
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td>
                        <input type="checkbox" id="lima" {{ $dokumen->tindakan_anestesi_epidural ? (in_array('lima',json_decode($dokumen->tindakan_anestesi_epidural->checklist))?'checked' : '') : '' }} class="form-control">
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td>
                        <input type="checkbox" id="enam" {{ $dokumen->tindakan_anestesi_epidural ? (in_array('enam',json_decode($dokumen->tindakan_anestesi_epidural->checklist))?'checked' : '') : '' }} class="form-control">
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td>
                        <input type="checkbox" id="tujuh" {{ $dokumen->tindakan_anestesi_epidural ? (in_array('tujuh',json_decode($dokumen->tindakan_anestesi_epidural->checklist))?'checked' : '') : '' }} class="form-control">
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td>
                        <input type="checkbox" id="delapan" {{ $dokumen->tindakan_anestesi_epidural ? (in_array('delapan',json_decode($dokumen->tindakan_anestesi_epidural->checklist))?'checked' : '') : '' }} class="form-control">
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td>
                        <input type="checkbox" id="sembilan" {{ $dokumen->tindakan_anestesi_epidural ? (in_array('sembilan',json_decode($dokumen->tindakan_anestesi_epidural->checklist))?'checked' : '') : '' }} class="form-control">
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td>
                        <input type="checkbox" id="sepuluh" {{ $dokumen->tindakan_anestesi_epidural ? (in_array('sepuluh',json_decode($dokumen->tindakan_anestesi_epidural->checklist))?'checked' : '') : '' }} class="form-control">
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td>
                        <input type="checkbox" id="sebelas" {{ $dokumen->tindakan_anestesi_epidural ? (in_array('sebelas',json_decode($dokumen->tindakan_anestesi_epidural->checklist))?'checked' : '') : '' }} class="form-control">
                    </td>
                  </tr>
                  <tr>
                    <td colspan="3">Dengan ini menyatakan bahwa saya perwakilan dari Rumah Sakit, telah menerangkan hal-hal diatas secara benar dan jelas dan memberikan kesempatan untuk bertanya dan berdiskusi</td>
                        <td id="box_ttd" onclick="open_modal_verifikasi()">
                            ...
                            @else
                                ...
                                @else
                                    <img src="{{ env('SMIS_UPLOAD_URL').'/' }}" style="height: 1cm; width: 1cm;" alt="">
                                @endif
                            @endif
                        <br>                            
                        <span id="namaterang_dokter">{{$dokumen->tindakan_anestesi_epidural&&$dokumen->id_verifikator == 0?'Ttd Dokter':''}}</span>                        
                        </td>
                  </tr>
                  <tr>
                    <td colspan="3">Dengan ini menyatakan saya/keluarga pasien telah menerima informasi dari dokter, sebagaimana diatas saya beri tanda tangan/paraf dikolom kananya serta telah diberi kesempatan untuk bertanya/berdiskusi dan telah memahaminya</td>
                        <td class="text-center" id="box_ttd" onclick="open_modal_ttd()">
                            @if($dokumen->tindakan_anestesi_epidural==""||is_null($dokumen->tindakan_anestesi_epidural->ttd_pengampu) || $dokumen->tindakan_anestesi_epidural->ttd_pengampu == "")
                            @else
                            <img src="{{ asset('signature_patient/'.$dokumen->tindakan_anestesi_epidural->ttd_pengampu) }}"
                                style="height: 1cm; width: 1cm;" alt="">
                            @endif
                        <br>                            
                        <span id="namaterang_pengampu">{{$dokumen->tindakan_anestesi_epidural&&$dokumen->tindakan_anestesi_epidural->ttd_pengampu?'':'Ttd Pasien'}}</span>                        
                        </td>
                  </tr>
                  <tr>
            ...
            <div class="col-md-12">    
                    ...
                    <table style="border-collapse: collapse; width:100%" class="mb-3">
                        <tr class="align-top">
                            ...
                            <td style="width: 3%;"> : </td>
                            <td style="width: 85%;">
                                <input type="text" name="pengampu" id="pengampu" value="{{$dokumen->tindakan_anestesi_epidural&&$dokumen->tindakan_anestesi_epidural->pengampu?$dokumen->tindakan_anestesi_epidural->pengampu:$dokumen->nama_pasien}}"
                        ...
                        <tr class="align-top">
                            ...
                            <td style="width: 3%;"> : </td>
                            <td style="width: 85%;">
                                <input type="text" name="tgl_lahir_pengampu" value="{{$dokumen->tindakan_anestesi_epidural&&$dokumen->tindakan_anestesi_epidural->tgl_lahir_pengampu?$dokumen->tindakan_anestesi_epidural->tgl_lahir_pengampu:$layanan->tgl_lahir}}"
                                    style="border:1px solid transparent; border-bottom: 2px dotted; background-color: transparent;"
                        ...
                        <tr class="align-top">
                            ...
                            <td style="width: 3%;"> : </td>
                            <td style="width: 85%;">
                                <input type="text" name="alamat_pengampu" value="{{$dokumen->tindakan_anestesi_epidural&&$dokumen->tindakan_anestesi_epidural->alamat_pengampu?$dokumen->tindakan_anestesi_epidural->alamat_pengampu:$layanan->alamat}}"
                                    style="border:1px solid transparent; border-bottom: 2px dotted; background-color: transparent;"
                    ...
                    <span class="group-checkbox">
                        <input type="checkbox" name="status_tindakan" {{ ($dokumen->tindakan_anestesi_epidural && isset($dokumen->tindakan_anestesi_epidural->status_tindakan)
                            && $dokumen->tindakan_anestesi_epidural->status_tindakan == 1) ? 'checked' : 'selected'}}
                            value="1" id=""> SETUJU /
                        <input type="checkbox" name="status_tindakan" {{ ($dokumen->tindakan_anestesi_epidural && isset($dokumen->tindakan_anestesi_epidural->status_tindakan)
                            && $dokumen->tindakan_anestesi_epidural->status_tindakan == 0) ? 'checked' : ''}}
                            value="0" id=""> TIDAK SETUJU
                    </span>
                    </span> untuk dilakukan tindakan 
                        
                    <span style="font-weight: bold">
                        <span class="group-checkbox">
                        <input type="checkbox" name="jenis_tindakan" {{ ($dokumen->tindakan_anestesi_epidural && isset($dokumen->tindakan_anestesi_epidural->status_tindakan)
                            && $dokumen->tindakan_anestesi_epidural->jenis_tindakan == 1) ? 'checked' : 'selected'}}
                            value="1" id=""> ANESTESI SPINAL / 
                        <input type="checkbox" name="jenis_tindakan" {{ ($dokumen->tindakan_anestesi_epidural && isset($dokumen->tindakan_anestesi_epidural->status_tindakan)
                            && $dokumen->tindakan_anestesi_epidural->jenis_tindakan == 0) ? 'checked' : ''}}
                            value="0" id=""> EPIDURAL
                    </span>
                    </span> kepada 
                    <span class="group-checkbox">
                    <input type="checkbox" name="hubungan" {{ ($dokumen->tindakan_anestesi_epidural && isset($dokumen->tindakan_anestesi_epidural->hubungan)
                        && $dokumen->tindakan_anestesi_epidural->hubungan == "Saya Sendiri") ? 'checked' : ''}} value="Saya Sendiri" id="hubungan"> Saya Sendiri / 
                    <input type="checkbox" name="hubungan" {{ ($dokumen->tindakan_anestesi_epidural && isset($dokumen->tindakan_anestesi_epidural->hubungan)
                        && $dokumen->tindakan_anestesi_epidural->hubungan == "Suami") ? 'checked' : ''}} value="Suami" id="hubungan"> Suami / 
                    <input type="checkbox" name="hubungan" {{ ($dokumen->tindakan_anestesi_epidural && isset($dokumen->tindakan_anestesi_epidural->hubungan)
                        && $dokumen->tindakan_anestesi_epidural->hubungan == "Istri") ? 'checked' : ''}} value="Istri" id="hubungan"> Istri /
                    <input type="checkbox" name="hubungan" {{ ($dokumen->tindakan_anestesi_epidural && isset($dokumen->tindakan_anestesi_epidural->hubungan)
                        && $dokumen->tindakan_anestesi_epidural->hubungan == "Anak") ? 'checked' : ''}} value="Anak" id="hubungan"> Anak /
                    <input type="checkbox" name="hubungan" {{ ($dokumen->tindakan_anestesi_epidural && isset($dokumen->tindakan_anestesi_epidural->hubungan)
                        && $dokumen->tindakan_anestesi_epidural->hubungan == "Ayah") ? 'checked' : ''}} value="Ayah" id="hubungan"> Ayah /
                    <input type="checkbox" name="hubungan" {{ ($dokumen->tindakan_anestesi_epidural && isset($dokumen->tindakan_anestesi_epidural->hubungan)
                        && $dokumen->tindakan_anestesi_epidural->hubungan == "Ibu") ? 'checked' : ''}} value="Ibu" id="hubungan"> Ibu /
                    <input type="checkbox" name="hubungan_hidden" id="hubungan_hidden" {{ ($dokumen->tindakan_anestesi_epidural && isset($dokumen->tindakan_anestesi_epidural->hubungan)
                        && $hubungan_lain=True) ? 'checked' : ''}} id="" hidden> 
                    <input type="text" name="hubungan2" id="hubungan_lain" {{ ($dokumen->tindakan_anestesi_epidural && isset($dokumen->tindakan_anestesi_epidural->hubungan)
                    ...
                    <p class="mt-5 text-right mb-5 tgl_ttd" style="margin-right:90px;">
                        Bekasi, <input type="text" class="datepicker" name="tanggal_ttd" value="{{$dokumen->tindakan_anestesi_epidural?date('d-m-Y',strtotime($dokumen->tindakan_anestesi_epidural->tanggal_diampu)):''}}"
                            style="width:100px;border:1px solid transparent; border-bottom: 2px dotted; width: 40% background-color: transparent;">
                        Jam <input type="text" class="timepicker" name="jam_ttd" value="{{$dokumen->tindakan_anestesi_epidural?date('H:i',strtotime($dokumen->tindakan_anestesi_epidural->tanggal_diampu)):''}}" 
                ...
                <div class="row justify-content-center mb-5">
                    <div class="col-md-6 text-center" id="box_ttd" onclick="open_modal_ttd()">
                        <p style="margin: 0;padding:0">Yang Menyatakan</p>
                            ...
                            @endif
                        <br>                            
                        <span id="namaterang_pengampu">{{$dokumen->tindakan_anestesi_epidural?$dokumen->tindakan_anestesi_epidural->pengampu:'Ttd & Nama Terang'}}</span>                        
                    </div>
