...
<form onsubmit="return cek_form(this)" id="form_persetujuan"
      action="{{ url('e_rekam_medis/detail/save_formulir_triage_terintegrasi') }}" method="post">
    @csrf
    <input type="hidden" name="dokumen" value="{{$dokumen->id}}">
    ...
    <input type="hidden" id="hide_checkbox_diagnosa" name="checkbox_diagnosa">
    <input type="hidden" id="hide_checkbox_emergent" name="checkbox_emergent">
    <input type="hidden" id="hide_checkbox_urgent" name="checkbox_urgent">
    <input type="hidden" id="hide_checkbox_not_urgent" name="checkbox_not_urgent">
    <input type="hidden" id="hide_checkbox_false_emergency" name="checkbox_false_emergency">
</form>
@if ($errors->any())
            ...
            <tr>
                <td style="width: 40%; padding: 10px;">
                    ...
                    <input onclick="cek_radio_cara_datang()"
                           ...
                           @else
                               {{ $dokumen->formulir_triage_terintegrasi ? ($dokumen->formulir_triage_terintegrasi->cara_datang == 'sendiri' ? 'checked' : '') : 'checked' }}
                           @endif
            ...
            <tr>
                <td style="width: 40%; padding: 10px;">
                    ...
                    <textarea id="alamat" class="form-control" style="margin-top: 5px"
                              rows="5">@if(old('alamat')){{ old('alamat') }}@else{{ $dokumen->formulir_triage_terintegrasi ? $dokumen->formulir_triage_terintegrasi->alamat : $layanan->alamat_pasien }}@endif</textarea>
                </td>
                <td style="width: 20%; padding: 10px; vertical-align: text-top">
                    Nama Pengantar :
        ...
        <table style="width: 100%" class="table_isian">
            ...
            <tr>
                ...
                <td style="width: 14%;">
                    Keadaan Umum : 
                    <input type="text"
                           value="{{ $layanan->tanda_vital ? $layanan->tanda_vital->keadaan_umum : '' }}"
                           id="keadaan_umum" style="border: 0; border-bottom: 2px dotted;">
                </td>
            ...
            <tr>
                ...
                <td style="width: 14%; vertical-align: text-top" rowspan="3">
                    ...
                    <input type="text"
                        ...
                        id="tensi" style="border: 0; border-bottom: 2px dotted; width: 60%"> mmHg
                    <br>
                    Suhu :
                    <br>
                    <input type="text"
                            value="{{ $layanan->tanda_vital ? $layanan->tanda_vital->suhu : '' }}"
                            id="suhu" style="border: 0; border-bottom: 2px dotted; width: 60%"> ᵒC
                    <br>
                    Frek. Nadi :
                    <br> 
                    <input type="text"
                        value="{{ $layanan->tanda_vital ? $layanan->tanda_vital->nadi : '' }}"
                        id="nadi" style="border: 0; border-bottom: 2px dotted; width: 60%"> x/menit
                    <br>
                    Frek. Napas :
                    <br>
                    <input type="text"
                        value="{{ $layanan->tanda_vital ? $layanan->tanda_vital->rr : '' }}"
                        id="rr" style="border: 0; border-bottom: 2px dotted; width: 60%"> x/menit
                    <br>
                    SpO<sub>2</sub> :
                    <br>
                    <input type="text"
                        value="{{ $layanan->tanda_vital ? $layanan->tanda_vital->spo2 : '' }}"
                        id="spo2" style="border: 0; border-bottom: 2px dotted; width: 60%"> %
                    <br>
                    <br>
                    Imunisasi : 
                    <br>
                    <input @if(old('imunisasi'))
                            ...
                            @else
                                {{ $dokumen->formulir_triage_terintegrasi ? ($dokumen->formulir_triage_terintegrasi->imunisasi == 'ya' ? 'checked' : '') : 'checked' }}
                            @endif
                    ...
                    <br>
                    <input type="checkbox" @if(isset($dokumen->formulir_triage_terintegrasi))
                        {{ in_array('tidak_ada',json_decode($dokumen->formulir_triage_terintegrasi->riwayat_alergi )) ? 'checked' : '' }}
                    @endif id="tidak_ada_alergi"> Tidak Ada
                    <br>
                    <input type="checkbox" @if(isset($dokumen->formulir_triage_terintegrasi))
...
<div class="row pb-5 pt-5" style="width:100%; margin-left:0;">
    <div style="text-align: center;" class="col-md-12">
        @if($dokumen->id_verifikator != 0)
            <a href="{{ url('e_rekam_medis/detail/pdf_formulir_triage_terintegrasi?dokumen='.$dokumen->id) }}"
...
<script
    src="https://cdnjs.cloudflare.com/ajax/libs/jquery.devbridge-autocomplete/1.4.7/jquery.autocomplete.min.js">
</script>
<script>
    ...
    function cek_form() {
        ...
        var checkbox_diagnosa = [];
        if ($('#checkbox_diagnosa').is(":checked")) {
            checkbox_diagnosa.push('checkbox_diagnosa')
        }

        var checkbox_emergent = [];
        if ($('#checkbox_emergent').is(":checked")) {
            checkbox_emergent.push('checkbox_emergent')
        }

        var checkbox_urgent = [];
        if ($('#checkbox_urgent').is(":checked")) {
            checkbox_urgent.push('checkbox_urgent')
        }

        var checkbox_not_urgent = [];
        if ($('#checkbox_not_urgent').is(":checked")) {
            checkbox_not_urgent.push('checkbox_not_urgent')
        }

        var checkbox_false_emergency = [];
        if ($('#checkbox_false_emergency').is(":checked")) {
            checkbox_false_emergency.push('checkbox_false_emergency')
        }

        var riwayat_alergi = [];
        if ($('#tidak_ada_alergi').is(":checked")) {
            riwayat_alergi.push('tidak_ada');
        }
        ...
        if ($('#resusitasi').is(":checked")) {
            jalan_nafas.push('resusitasi');
        }
        if ($('#emergent').is(":checked")) {
            jalan_nafas.push('emergent');
        }
        if ($('#urgent').is(":checked")) {
            jalan_nafas.push('urgent');
        }
        if ($('#not_urgent').is(":checked")) {
            jalan_nafas.push('not_urgent');
        }
        if ($('#false_emergency').is(":checked")) {
            jalan_nafas.push('false_emergency');
        }
        ...
        if ($('#satu').is(":checked")) {
            pernafasan.push('satu');
        }
        if ($('#dua').is(":checked")) {
            pernafasan.push('dua');
        }
        if ($('#tiga').is(":checked")) {
            pernafasan.push('tiga');
        }
        if ($('#empat').is(":checked")) {
            pernafasan.push('empat');
        }
