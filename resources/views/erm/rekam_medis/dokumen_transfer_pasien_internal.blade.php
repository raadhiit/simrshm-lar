...
<div class="modal fade" id="modal_petugas2" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <form method="post" action="{{ url('e_rekam_medis/rekam_medis/verifikasi_petugas_penyerahan') }}">
                @csrf
                <input type="hidden" name="dokumen" value="{{$dokumen->id}}">
                <div class="modal-body">
                    <div class="form-group">
...
<form onsubmit="return cek_form(this)" id="form_persetujuan" action="{{ url('e_rekam_medis/rekam_medis/save_dokumen_transfer_pasien_internal') }}" method="post">
    @csrf
    <input type="hidden" name="dokumen" value="{{$dokumen->id}}">
                ...
                <tr>
                    ...
                    <td style="width: 85%;">
                        <select id="kesadaran" class="form-control">
                            ...
                            <option @if(old('kesadaran')) {{ old('kesadaran') == 'composmentis' ? 'selected' : '' }} @else {{ $dokumen->dokumen_transfer_pasien_internal ? $dokumen->dokumen_transfer_pasien_internal->kesadaran == 'composmentis' ? 'selected' : '' : '' }} @endif value="composmentis">Composmentis</option>
                            <option @if(old('kesadaran')) {{ old('kesadaran') == 'sompolen' ? 'selected' : '' }} @else {{ $dokumen->dokumen_transfer_pasien_internal ? $dokumen->dokumen_transfer_pasien_internal->kesadaran == 'sompolen' ? 'selected' : '' : '' }} @endif value="sompolen">Sompolen</option>
                            <option @if(old('kesadaran')) {{ old('kesadaran') == 'sopor' ? 'selected' : '' }} @else {{ $dokumen->dokumen_transfer_pasien_internal ? $dokumen->dokumen_transfer_pasien_internal->kesadaran == 'sopor' ? 'selected' : '' : '' }} @endif value="sopor">Sopor</option>
                            <option @if(old('kesadaran')) {{ old('kesadaran') == 'coma' ? 'selected' : '' }} @else {{ $dokumen->dokumen_transfer_pasien_internal ? $dokumen->dokumen_transfer_pasien_internal->kesadaran == 'coma' ? 'selected' : '' : '' }} @endif value="coma">Coma</option>
                        </select>
                    </td>
                </tr>
                <tr>
...
<div class="row pb-5 pt-5" style="width:100%; margin-left:0;">
    <div style="text-align: center;" class="col-md-12">
        @if($dokumen->id_verifikator != 0)
            <a href="{{ url('e_rekam_medis/rekam_medis/pdf_dokumen_transfer_pasien_internal?dokumen='.$dokumen->id) }}"
