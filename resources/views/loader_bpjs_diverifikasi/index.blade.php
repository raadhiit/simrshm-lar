...
@section('content')
    ...
    <div class="modal fade" id="uploadModal" tabindex="-1" role="dialog" aria-labelledby="uploadModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('loader_bpjs_diverifikasi.import') }}" method="POST" enctype='multipart/form-data'>
                    @csrf
                    ...
                    <div class="modal-footer">
                        <a class="btn btn-info mr-auto"
                            href="{{ asset('template/Template Loader Klaim BPJS Diverifikasi.xlsx') }}"
                            data-toggle="tooltip" title="Download Template Excel">Template</a>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header" style="margin-top: -15px;">
                    <h5 class="modal-title text-primary" id="exampleModalLongTitle">Form Data Loader BPJS Diverifikasi</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{ route('loader_bpjs_diverifikasi.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
    ...
    <div class="main-content">
        <section class="section">
            <div class="section-header">
                <h1>Data Loader BPJS Diverifikasi</h1>
            </div>
            @if ($errors->any())
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped" style="width:100%;">
                                ...
                                <tbody>
                                    @inject('loaderService', 'App\Services\LoaderBpjsDIverifikasiService')
                                        ...
                                        <tr class="">
                                            ...
                                            <td style="width:5%;">
                                                <div style="display: flex; flex-direction: row;">
                                                    ...
                                                    @endif
                                                    @if ($dt->user_upload != null && $dt->tanggal_upload != null)
                                                        <a href="{{ route('loader_bpjs_diverifikasi.detail', [$dt->id, 'uri' => $dt->jenis_layanan]) }}"
                                                    ...
                                                    @if ($dt->user_hapus == null && $dt->tanggal_hapus == null && $status != 'Diterima' && $status != 'Ditolak')
                                                        @if ($dt->user_upload != null &&
                                                            $dt->tanggal_upload != null &&
                                                            ($status == 'Belum Generate' || $status == 'Gagal Generate'))
                                                        ...
                                                        @if ($dt->id_draft_jurnal == null)
                                                            <form
                                                                action="{{ route('loader_bpjs_diverifikasi.destroy', $dt->id) }}"
    ...
    <script>
        function directPage() {
            window.location.href = "{{ route('loader_bpjs_diverifikasi.index') }}";
        }
        ...
        function generateJurnal(id) {
            ...
            if (konfirmasi) {
                ...
                var url = "{{ route('loader_bpjs_diverifikasi.data_generate', ':id') }}";
        ...
        function postToDraftDetail(dataDetail, idDraft) {
            ...
            for (var i = 0; i < dataDetail.length; i++) { // Using looping for checkin dataDetail count
                ...
                $.ajax({
                    url: "{{ route('loader_bpjs_diverifikasi.post_detail_draft') }}",
                    type: 'POST',
                    data: dataToSend,
                    headers: {
                    ...
                    success: function(response) {
                        ...
                        if (response.status == 'diajukan') {
                            inputMessageModal('Berhasil input data detail sep :' + response.sep, 'black');
                            hideLoading();
                            $('#reportModal').modal('show');
                        } else {
                            messageProgress('Proses input data detail sep :' + response.sep + '.....');
                            inputMessageModal('Berhasil input data detail sep :' + response.sep, 'black');
                        }
                    },
                    error: function(xhr, status, error) {
        ...
        function postToDraftHeader(dataHeader) {
            return new Promise(function(resolve, reject) {
                ...
                $.ajax({
                    url: "{{ route('loader_bpjs_diverifikasi.post_draft') }}",
                    type: 'POST',
                    data: dataToSend,
                    headers: {
                        'X-CSRF-TOKEN': csrfToken
