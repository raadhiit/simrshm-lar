...
@section('content')
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header" style="margin-top: -15px;">
                    <h5 class="modal-title text-primary" id="exampleModalLongTitle">Tambah Status Tenaga</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>>
                <form action="{{ route('status_tenaga.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
    ...
    <div class="modal fade" id="modalEdit" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header" style="margin-top: -15px;">
                    <h5 class="modal-title text-primary" id="exampleModalLongTitle">Ubah Status Tenaga</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>>
                <form action="{{ route('status_tenaga.update', 0) }}" method="post" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    ...
                    <div class="card-body">
                        <div class="card-header" style="margin-left: -25px;">
                            <div class="card-header-form">
                                <form method="get" action="{{ route('status_tenaga.search') }}">
                                            ...
                                            <tr class="text-center">
                                                ...
                                                <td style="width:5%;">
                                                    <div style="display: flex; flex-direction: row;">
                                                        ...
                                                        <form action="{{ route('status_tenaga.destroy', $dt->id) }}"
                                                            method="post" enctype="multipart/form-data">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button
                                                                onclick="return confirm('Yakin melanjutkan hapus data {{ $dt->status_tenaga }}?')"
    ...
    <script>
        function showEditModal(param) {
            var url = "{{ route('status_tenaga.edit', ':id') }}";
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#nama_edit').val(response.status_tenaga);
        ...
        function downloadExcel() {
            ...
            var url = "{{ route('status_tenaga.download') }}/?keyword=" + keyword;
