...
use App\Models\Smis_Doc_Tindakan_Anestesi_Epidural;
...
class TindakanAnestesiEpiduralService
{
    function data($req)
    {
        $data['dokumen'] = DokumenKunjungan::with('tindakan_anestesi_epidural')
    ...
    function create($req)
    {
        $itas = Smis_Doc_Tindakan_Anestesi_Epidural::updateOrCreate([
            ...
            'jenis_tindakan' => $req->jenis_tindakan,
            ...
            'tanggal_diampu' => ($req->tanggal_ttd ? date('Y-m-d', strtotime($req->tanggal_ttd)) : date('Y-m-d')) . ' ' . ($req->jam_ttd ? $req->jam_ttd . ':00' : date('H:i:s')),
        ]);
    ...
    function save_signature($data)
    {
        try {
            ...
            Smis_Doc_Tindakan_Anestesi_Epidural::where('id_dokumen', $data->dokumen)->update([
    ...
    function verifikasi_dokumen($data){
        ...
        if($data->verif == 1){
            $verif = Smis_Doc_Tindakan_Anestesi_Epidural::where([
